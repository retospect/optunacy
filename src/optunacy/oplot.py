        ...
        for trial in self.study.trials:
            if trial.state == optuna.trial.TrialState.COMPLETE:
                ...
                if name in combined_dict:
                    ...
                    raise ValueError(f"Parameter '{name}' not found in trial parameters or user attributes.")
        return values 
...
def plot(x_name, y_name, z_name=None, x_range=None, y_range=None):
    ...
    if z_name:
        ...
        Z = scipy.interpolate.griddata((x_values, y_values), z_values, (X, Y), method='cubic')
